...
const char *http_request_line(int fd, char *reqpath, char *env, size_t *env_len)
{
    ...
    printf("in http_request_line: buf after http_read_line: %s\n", buf);
...
valid_cgi_script(struct stat *st)
{
    ...
    {
        printf("in valid_cgi_script 1\n");
        return 0;
        }

    if (!(st->st_mode & S_IXUSR))
    {
        printf("in valid_cgi_script 2\n");
        return 0;
        }

    if (cgi_uid >= 0 && cgi_gid >= 0) {
        printf("in valid_cgi_script 3\n");
        printf("st->st_uid %d cgi_uid: %d\n", st->st_uid,cgi_uid);
        printf("st->st_gid %d cgi_gid: %d\n", st->st_gid,cgi_gid);
...
void http_serve_file(int fd, const char *pn)
{
    ...
    if (getenv("PATH_INFO")) {
        ...
        printf("in http_server_file: env: %s\n", getenv("PATH_INFO"));
        printf("in http_server_file: pn: %s\n", pn);
